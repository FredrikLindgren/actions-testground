...
when a \ttref{COPY}, \ttref{COPY!EXISTING}, \ttref{APPEND},
\ttref{APPEND!COL}, \ttref{EXTEND!TOP}, \ttref{EXTEND!BOTTOM},
...
checked against the \ttref{when} clauses separately. \\
  ...
  or & \DEFINE{IF!EXISTS} & True only if the file exists. For
  \ttref{COPY}, this check is done with \ttref{FILE!EXISTS}. For
  \ttref{COPY!EXISTING}, \ttref{COPY!EXISTING!REGEXP},
  \ttref{COPY!RANDOM}, \t{APPEND}, \ttref{APPEND!COL},
  \ttref{EXTEND!TOP}, \ttref{EXTEND!BOTTOM},
...
function and 0 if string is not the name of any defined function. Note
that both 1 and 2 are true in a boolean context. In the case of, for
example, dimorphic functions, where both an action function and a
patch function exist of the same name, the return value is 1. \\
